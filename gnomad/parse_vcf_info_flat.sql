...
-- **********************************************************************************************
-- THIS DIFFERES FROM THE VERSION IN 1000Genomes SCHEMA, WHICH HANDLE MULTIPLE ALT VALUES PER POS
-- **********************************************************************************************
-- into a JSON representation for easier querying (or to prepare for column extraction)
-- 
-- Appropriate for some annotation VCFs that never have multiple-alt alleles such as GNOMAD
--
-- Inputs:
...
--      select parse_vcf_info('AC=2130;AF=0.425319;AN=5008;NS=2504;DP=103152;EAS_AF=0.3363;AMR_AF=0.3602;AFR_AF=0.4909;EUR_AF=0.4056;SAS_AF=0.4949;AA=|||unknown(NO_COVERAGE);VT=INDEL'
--                          , get_info_headers('chr1'));
--
--      or applied to an entire table:
--      select parse_vcf_info(info, get_info_headers('chr1')) from variants_raw ;
-- *************************************************************************************************************
-- TODO:  Arrayify bin_freq fields using | as separator
--        Parse sub-elements of the VEP array
use schema GNOMAD;
