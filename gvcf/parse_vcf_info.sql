-- Scalar UDF to convert the INFO field from VCF entries into a JSON representation for easier querying (or to prepare for column extraction)
--
-- Inputs:
...
CREATE OR REPLACE FUNCTION parse_vcf_info(info varchar, struct varchar)
  RETURNS variant
  LANGUAGE JAVASCRIPT
  AS '
    var setup = function() {
    ...
    infoArray.forEach((item) => {
        ...
        else if (structMap[theKey].number == "1") {
